...
if (blockName) {
  ...
  mkdirp(dirPath, (err) => {                                           // создаем
    ...
    if (err) {
      console.error(`[Elf-decor] Отмена операции: ${err}`);
    }

    // Нет ошибки, поехали!
    else {
      console.log(`[Elf-decor] Создание папки ${dirPath} (если отсутствует)`);
      ...
      extensions.forEach((extention) => {
        ...
        if (extention === 'scss') {
          ...
          if (!hasThisBlock) {
            ...
            fileCreateMsg = '[Elf-decor] Подключение блока добавлено в projectConfig.json';
          }
        }
        ...
        // Если нужна подпапка для картинок
          ...
          if (fileExist(imgFolder) === false) {
            mkdirp(imgFolder, (err) => {
              ...
              else console.log(`[Elf-decor] Создание папки: ${imgFolder} (если отсутствует)`);
            });
          } else {
            console.log(`[Elf-decor] Папка ${imgFolder} НЕ создана (уже существует) `);
          }
        }
        ...
        if (fileExist(filePath) === false && extention !== 'img') {
          fs.writeFile(filePath, fileContent, (err) => {
            if (err) {
              return console.log(`[Elf-decor] Файл НЕ создан: ${err}`);
            }
            console.log(`[Elf-decor] Файл создан: ${filePath}`);
            if (fileCreateMsg) {
              console.warn(fileCreateMsg);
            }
          });
        } else if (extention !== 'img') {
          console.log(`[Elf-decor] Файл НЕ создан: ${filePath} (уже существует)`);
        }
      });
    }
  });
} else {
  console.log('[Elf-decor] Отмена операции: не указан блок');
}
